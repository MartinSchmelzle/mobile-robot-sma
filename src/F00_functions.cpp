...
//This function converts SMA input into viable waypoints
...
void checkPoints(const double XS[6], const double YS[6], SMAsol_struct* sol, uint8_t &k) {
    ...
    double e = 1e-2;
    int p = 1;
    sol->internalpath.startTurn = 0;
    sol->internalpath.endTurn = 0;
    while (p) {
        p = 0;
        for (int i = 1; i < k; ++i) {
            if ((std::abs(sol->XS[i] - sol->XS[i - 1]) < e) && (std::abs(sol->YS[i] - sol->YS[i - 1]) < e)) {
                push_to_left(sol->XS, i);
                push_to_left(sol->YS, i);
                k--;
                p = 1;
                if (i == 1) {
                    sol->internalpath.startTurn = 1;
                } else if (i == k) {
                    sol->internalpath.endTurn = 1;
                }
            }
        }
        for (int i = 2; i < k - 1; ++i) {
            if ((std::abs(sol->XS[i] - sol->XS[i - 2]) < e) && (std::abs(sol->YS[i] - sol->YS[i - 2]) < e)) {
                push_to_left(sol->XS, i);
                push_to_left(sol->YS, i);
                k--;
                p = 1;
    ...
    for (int i = 0; i < k - 1; ++i) {
        XSV_out[i] = sol->XS[i + 1] - sol->XS[i];
        YSV_out[i] = sol->YS[i + 1] - sol->YS[i];
    ...
    if (sol->XS[k - 1] != temp[0] || sol->YS[k - 1] != temp[1]) {
        k++;
        sol->XS[k - 1] = temp[0];
        sol->YS[k - 1] = temp[1];
    }
...
void getAngRad(const std::array<double, 10> XS,const std::array<double, 10> YS,const int i,const double g_s, double& g_a, double& g_r) {
    // Vektoren AB und BC
    const double vectorBA[2] = { -(XS[i + 1] - XS[i]), -(YS[i + 1] - YS[i]) };
    const double vectorBC[2] = { XS[i + 2] - XS[i + 1], YS[i + 2] - YS[i + 1] };

    // Berechne das Skalarprodukt von AB und BC
    const double dotProduct = vectorBA[0] * vectorBC[0] + vectorBA[1] * vectorBC[1];

    // Berechne die Längen der Vektoren
    // Berechne den Winkel in Bogenmaß (Radian) zwischen AB und BC
    const double lengthBA = std::sqrt(vectorBA[0] * vectorBA[0] + vectorBA[1] * vectorBA[1]);
    const double lengthBC = std::sqrt(vectorBC[0] * vectorBC[0] + vectorBC[1] * vectorBC[1]);
...
void findMid(const std::array<double, 10> XS,const std::array<double, 10> YS, double dis[],const double g_r,const double g_s,const int i, double* m0, double* m1) {
    ...
    if (g_r == 0) {
        ...
        *m1 = YS[i] + (dis[i] - g_s) / dis[i] * vecAB[1];
        ...
        //vereinfachte Version davon:
        double length_vecAB=std::sqrt(vecAB[1]*vecAB[1] + vecAB[0]*vecAB[0]);
        double a=(g_r/length_vecAB) * (-vecAB[1]);
        double b=(g_r/length_vecAB) * vecAB[0];
        ...
        *m1 = YS[i] + (dis[i] - g_s) / dis[i] * vecAB[1] + b;
        ...
        //Wir berechnen hier das Kreuzprodukt und skalieren den Vektor dann auf die erforderliche Größe. In den Kommentaren ist die mathematisch nachvollziehbarere Version, im Code ist das ganze vereinfacht
        ...
        double length_vecBC=std::sqrt(vecBC[1]*vecBC[1] + vecBC[0]*vecBC[0]);
        double c=(g_r / length_vecBC) * (-vecBC[1]);
        double d=(g_r / length_vecBC) * (vecBC[0]);
...
std::array<double,2> &startpoint, std::array<double,2> &endpoint) {
   ...
   if (g_r == 0) {
       ...
       //double ay = (g_r / abs_cross) * cross_product[1];
        double length_vecAB=std::sqrt(vecAB[1]*vecAB[1] + vecAB[0]*vecAB[0]);
        double ax=(g_r/length_vecAB) * (-vecAB[1]);
        double ay=(g_r/length_vecAB) * vecAB[0];
       ...
       //double by = (g_r / std::sqrt(cross_product_bc[0] * cross_product_bc[0] + cross_product_bc[1] * cross_product_bc[1])) * cross_product_bc[1];
        double length_vecBC=std::sqrt(vecBC[1]*vecBC[1] + vecBC[0]*vecBC[0]);
        double bx=(g_r / length_vecBC) * (-vecBC[1]);
        double by=(g_r / length_vecBC) * (vecBC[0]);
...
double discretize(SMAsol_struct* sol, const int k, const int n,const double distance_discrete) 
{
    ...
    for(int i=0;i<4;i++)
    {
        ...
        //write lineVector and lineVectorLength into sol struct for later usage
        sol->internalpath.lineVector[i] = lineVector;
        sol->internalpath.lineLength[i]=lineVectorLength;
    ...
    lineVector[1]/=lineVectorLength; //create unit vector
    //write lineVector and lineVectorLength into sol struct for later usage
    sol->internalpath.lineVector[4] = lineVector;
    sol->internalpath.lineLength[4]=lineVectorLength;
    //runner follows straight line, entries are added to xx,yy
    int n_linepoints = floor(lineVectorLength/distance_discrete);
    for(int j=0;j<n_linepoints;j++)
    {
...
float getViolationRect(const model_struct* model,const SMAsol_struct* sol,const int k) {
    ...
    float Violation=0;
    const float add_collision=100.0/amount_points;//100.0 is important, otherwise we are dividing integers by integers, which returns wrong results
...
float getViolationCircBoundary(const model_struct* model,const SMAsol_struct* sol) {
    ...
    int i=0;
    float Violation=0;
    float add_collision=100.0/amount_points;//100.0 is important, otherwise we are dividing integers by integers, which returns wrong results
    bool collision;
...
float getViolationChargingStation(const SMAsol_struct* sol, const model_struct* model, const int k) {
    ...
    std::vector<float> xx=sol->xx;
    std::vector<float> yy=sol->yy;
    bool collision;
