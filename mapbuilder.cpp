...
class obstacle {
    ...
    unsigned scale; //scale: How many pixels (or units in an svg file) are in one meter?
    RGBColor color;
    std::string label;
...
class map {
    ...
    int createsvgmap() {
        ...
        for(auto& charger: ChargerVector){
            ...
            point pos={charger.getpos()[0]*charger.getscale()-width/2,charger.getpos()[1]*charger.getscale()-height/2};
