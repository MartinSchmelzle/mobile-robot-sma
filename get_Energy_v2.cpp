...
//print out result of kinematics calculation
...
void get_kinematics_line(kinematics_time* kin, double lineVector[2], const bool breakatend)
{
    //step 1: read out the required data from h, set everything up
    const int time_size=10000;

    //step 2.1: Since we have multiple line segments in sequence, we might not need to start at entry 0, but instead another entry => find that entry
    ...
    const double lineLength = std::sqrt(lineVector[0]*lineVector[0] + lineVector[1]*lineVector[1]);
    ...
    //step 3.5: At the end, the result is between the i-1th and ith entry. Find out the time at which the robot reaches the line's end. This is not true if breakatend=1
    if (!breakatend){
...
void get_kinematics_arc(kinematics_time* kin, const double arc_break_length, const double centerPoint[2], const double endpoint[2], const double radius)
{
    ...
    double vec[2]={arcLength-arc_break_length,0};
    get_kinematics_line(kin,vec,0);
...
double getEnergy_v2(SMAsol_struct* sol, kinematics_time* kin)
{
    ...
    for(int i=0;i<4;i++)
    {
        if(i==0)
        {
        lineVector[0] = sol->internalpath.arc_startpoints_x[0] - sol->XS[0];
        lineVector[1] = sol->internalpath.arc_startpoints_y[0] - sol->YS[0];
        ...
        get_kinematics_line(kin, lineVector, 0);
    ...
    get_kinematics_line(kin, lineVector, 1);
    ...
    const double P_idle = 64.6; //in W
    double E_need = 0;
    double mass;
    const std::array<double,8> p = {0.244,2.77,1.94,-5.26,0.0934,0.0911,0.00129,-0.00626}; //values for combined drive
    for(int i=0;i<find_i_start(kin->a_array);i++)
    {
        if(std::isnan(kin->v_array[i])){kin->v_array[i]=0;}
        if(std::isnan(kin->a_array[i])){kin->a_array[i]=0;}
        double P_komb = P_idle +//equation for combined drive
                    p[0]*kin->v_array[i] * ( mass * (p[1]+p[2] * kin->a_array[i]) + p[3] * std::pow(kin->v_array[i],2))+
        ...
        E_need += P_komb * kin->delta_t;
    }

    return std::abs(E_need); //Dummy
}